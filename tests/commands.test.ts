...
async function verify_branch_history(dataDir: string, currentRegistry: ProjectRegistry, projectId: string) {
  const project = currentRegistry.getById(projectId);
  if (!project) {
    return
  }
  for (const branch of project.branches) {
    ...
    let currentVersion: HistoryItem = data[branch.version.major][branch.version.minor][branch.version.path];
    let prevVersion: [number, number, number] | undefined;
    do {
      if (!prevVersion) {
        expect(currentVersion.target.value).toMatchObject(
      ...
      prevVersion = currentVersion.previous;
      if (prevVersion) {
        currentVersion = data[prevVersion[0]][prevVersion[1]][prevVersion[2]];
      }
...
describe("Create Application",   () => {
  ...
  test('Create new application', async () => {
    ...
    const [currentRegistry, project] = await create<Project>(commandParams);
    await verify_project_store(config.dataDir, currentRegistry);
    await verify_branch_history(config.dataDir, currentRegistry, project.id);
  });
  test("Create branch for application", async () => {
    ...
    await verify_branch_history(config.dataDir, registry, project.id);
